...
from app.models import CalendarEvent, CalendarRange, CalendarSnapshot, CalendarColor, EventCategory, HouseholdCalendar
...
class MockCalendarProvider:
    ...
    def _seed_events(self) -> list[CalendarEvent]:
        ...
        def event(event_id: str, calendar_id: str, title: str, start: datetime, end: datetime, location: str | None = None, categories: list[EventCategory] | None = None) -> CalendarEvent:
            return CalendarEvent(id=event_id, calendar_id=calendar_id, title=title, starts_at=start, ends_at=end, location=location, categories=categories or [])
        ...
        return [
            event("school-dropoff", "family", "School drop-off", at(0, 7, 45), at(0, 8, 15), categories=[school]),
            event("standup", "alex", "Product stand-up", at(0, 9), at(0, 9, 30), "Study nook", [work]),
            event("swim", "jordan", "Swim practice", at(0, 16), at(0, 17, 15), "Riverside pool", [sports]),
            event("dinner", "home", "Taco night", at(0, 18, 30), at(0, 20)),
            event("dentist", "jordan", "Dentist appointment", at(1, 10), at(1, 11), "Cedar Street Dental", [medical]),
            event("library", "family", "Return library books", at(2, 15), at(2, 15, 45), "Main library", [school]),
            event("soccer", "jordan", "Soccer match", at(3, 9), at(3, 10, 30), "North field", [sports, EventCategory(id="school", name="School", color="blue")]),
            CalendarEvent(id="camp", calendar_id="family", title="School break", starts_at=at(5, 0), ends_at=at(8, 0), all_day=True, categories=[school]),
            event("groceries", "home", "Grocery run", at(6, 11), at(6, 12)),
            event("birthday", "family", "Mia's birthday", at(10, 0), at(10, 0) + timedelta(days=1), categories=[birthday]),
        ]
