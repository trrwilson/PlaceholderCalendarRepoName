...
function CompactEvent({ event, calendar, onSelect, colorMode }: EventProps & { colorMode: SemanticColorMode }) { return <button className={`compact-event ${semanticEventClass(event, calendar, colorMode)}`} style={semanticEventStyle(event, colorMode)} onClick={() => onSelect(event)}><span><strong>{event.title}</strong><small><IdentityMarker calendar={colorMode === 'category-first' ? calendar : undefined} /><CategoryMarkers categories={colorMode === 'people-first' ? event.categories : []} />{event.all_day ? 'All day' : formatEventTime(event.starts_at)}</small></span></button> }
