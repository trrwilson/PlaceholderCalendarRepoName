...
function CategoryMarkers({ categories = [] }: { categories?: EventCategory[] }) { if (!categories.length) return null; return <span className="category-markers" aria-label={categories.map((category) => category.name).join(', ')}>{categories.map((category) => <i className="category-dot" style={categoryVar(category.color)} title={category.name} key={category.id} />)}</span> }
function primaryCategory(event: CalendarEvent, colorMode: SemanticColorMode) { return colorMode === 'category-first' ? event.categories?.[0] : undefined }
function semanticEventClass(event: CalendarEvent, calendar: Calendar | undefined, colorMode: SemanticColorMode) { return primaryCategory(event, colorMode) ? 'category-dominant' : colorClass(calendar?.color ?? 'coral') }
function semanticEventStyle(event: CalendarEvent, colorMode: SemanticColorMode) { return categoryVar(primaryCategory(event, colorMode)?.color) }
...
function LargeEvent({ event, calendar, onSelect, colorMode, past = false }: EventProps & { colorMode: SemanticColorMode; past?: boolean }) { return <button className={`large-event ${semanticEventClass(event, calendar, colorMode)} ${past ? 'past' : ''}`} style={semanticEventStyle(event, colorMode)} onClick={() => onSelect(event)}><span className="large-event-time">{event.all_day ? 'ALL DAY' : formatEventTime(event.starts_at)}</span><span className="large-event-main"><strong>{event.title}</strong>{event.location && <small>{event.location}</small>}</span><span className="event-owner"><IdentityMarker calendar={calendar} /><CategoryMarkers categories={colorMode === 'people-first' ? event.categories : []} />{calendar?.name}</span><span className="event-arrow">›</span></button> }
function CompactEvent({ event, calendar, onSelect, colorMode }: EventProps & { colorMode: SemanticColorMode }) { return <button className="compact-event" onClick={() => onSelect(event)}><span className={`compact-dot ${colorMode === 'category-first' ? semanticEventClass(event, calendar, colorMode) : colorClass(calendar?.color ?? 'coral')}`} style={colorMode === 'category-first' ? semanticEventStyle(event, colorMode) : undefined} /><span><strong>{event.title}</strong><small><IdentityMarker calendar={colorMode === 'category-first' ? calendar : undefined} /><CategoryMarkers categories={colorMode === 'people-first' ? event.categories : []} />{event.all_day ? 'All day' : formatEventTime(event.starts_at)}</small></span></button> }
function WeekEvent({ event, calendar, onSelect, colorMode }: EventProps & { colorMode: SemanticColorMode }) { const start = new Date(event.starts_at); const end = new Date(event.ends_at); const top = ((start.getHours() + start.getMinutes() / 60) - 7) / 14 * 100; const height = Math.max(((end.getTime() - start.getTime()) / 3_600_000) / 14 * 100, 8); return <button className={`week-event ${semanticEventClass(event, calendar, colorMode)}`} style={{ top: `${Math.max(top, 1)}%`, height: `${Math.min(height, 97 - Math.max(top, 1))}%`, ...semanticEventStyle(event, colorMode) }} onClick={() => onSelect(event)}><strong>{event.title}</strong><span><IdentityMarker calendar={colorMode === 'category-first' ? calendar : undefined} /><CategoryMarkers categories={colorMode === 'people-first' ? event.categories : []} />{event.all_day ? 'All day' : formatEventTime(event.starts_at)}</span></button> }
function EventChip({ event, calendar, onSelect, colorMode }: EventProps & { colorMode: SemanticColorMode }) { return <button className={`event-chip ${semanticEventClass(event, calendar, colorMode)} ${event.all_day ? 'all-day' : ''}`} style={semanticEventStyle(event, colorMode)} onClick={() => onSelect(event)}><span className="event-time">{event.all_day ? 'ALL DAY' : formatEventTime(event.starts_at)}</span><strong>{event.title}</strong><span><IdentityMarker calendar={colorMode === 'category-first' ? calendar : undefined} /><CategoryMarkers categories={colorMode === 'people-first' ? event.categories : []} /></span></button> }
