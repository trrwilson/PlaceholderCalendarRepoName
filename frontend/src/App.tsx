...
function App() {
  ...
  useEffect(() => {
    const range = rangeForView(mode, viewDate, now)
  ...
  return (
    <main className="kiosk-shell">
      ...
      <section className="view-frame">
        {mode === 'home' && <HomeView now={now} todayEvents={todayEvents} upcoming={nextEvents} calendarById={calendarById} onSelect={setSelectedEvent} colorMode={colorMode} />}
        {mode === 'week' && <WeekView viewDate={viewDate} now={now} events={visibleEvents} calendarById={calendarById} onSelect={setSelectedEvent} onNavigate={navigate} colorMode={colorMode} />}
        {mode === 'month' && <MonthView viewDate={viewDate} now={now} events={visibleEvents} calendarById={calendarById} onSelect={setSelectedEvent} onNavigate={navigate} colorMode={colorMode} />}
      </section>
...
function MonthView({ viewDate, now, events, calendarById, onSelect, onNavigate, colorMode }: { viewDate: Date; now: Date; events: CalendarEvent[]; calendarById: Map<string, Calendar>; onSelect: (event: CalendarEvent) => void; onNavigate: (amount: number) => void; colorMode: SemanticColorMode }) {
...
function rangeForView(mode: ViewMode, date: Date, now: Date) { if (mode === 'home') return { start: startOfDay(now), end: addDays(now, 14) }; if (mode === 'week') { const start = startOfWeek(date); return { start, end: addDays(start, 6) } } const start = new Date(date.getFullYear(), date.getMonth(), 1); const end = new Date(date.getFullYear(), date.getMonth() + 1, 0); return { start: addDays(start, -start.getDay()), end: addDays(end, 6 - end.getDay()) } }
function startOfDay(date: Date) { return new Date(date.getFullYear(), date.getMonth(), date.getDate()) }
function startOfWeek(date: Date) { const start = startOfDay(date); start.setDate(start.getDate() - start.getDay()); return start }
...
function buildCalendarDays(month: Date) { const start = new Date(month.getFullYear(), month.getMonth(), 1); start.setDate(start.getDate() - start.getDay()); return Array.from({ length: 42 }, (_, index) => addDays(start, index)) }
