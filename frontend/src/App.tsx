  ...
  return (
    <main className="kiosk-shell">
      ...
      <section className="view-frame">
        {mode === 'home' && <HomeView now={now} todayEvents={todayEvents} upcoming={nextEvents} calendarById={calendarById} onSelect={setSelectedEvent} colorMode={colorMode} calendarAlert={authNeedsSetup ? { account: auth?.account ?? null, onConnect: () => { setAddingCalendar(false); setConnectOpen(true) } } : null} />}
      ...
      {connectOpen && auth && <CalendarConnect auth={auth} addingCalendar={addingCalendar} onStart={beginConnect} onCancel={cancelConnect} onClose={() => setConnectOpen(false)} />}
...
// One "main" association per classification, regardless of how many the backend attaches: first category wins.
function mainCategory(event: CalendarEvent, colorMode: SemanticColorMode) { return colorMode === 'category-first' ? event.categories?.[0] : undefined }
function semanticEventClass(event: CalendarEvent, calendar: Calendar | undefined, colorMode: SemanticColorMode) { return mainCategory(event, colorMode) ? 'category-dominant' : colorClass(calendar?.color ?? 'coral') }
// The secondary association — drawn as the left-edge triangle. Present only when the event carries both a
// calendar and a category, i.e. there is a genuine "other" classification to show.
function eventAccent(event: CalendarEvent, calendar: Calendar | undefined, colorMode: SemanticColorMode): { color: string; label: string } | undefined {
  ...
  if (!category || !calendar) return undefined
  return colorMode === 'category-first'
    ? { color: PALETTE_TOKEN[calendar.color] ?? PALETTE_TOKEN.coral, label: calendar.name }
